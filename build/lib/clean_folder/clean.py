...
def normalize(name: str) -> str:
    translate_name = re.sub(r'[^1-9a-zA-Z.]', '_', name.translate(TRANS))
...
def scan(folder: Path):
    for item in folder.iterdir():
        ...
        if item.is_dir():  # перевіряємо чи обєкт папка
            if item.name not in ('applications', 'archives', 'video', 'audio', 'documents', 'images', 'MY_OTHER'):
                FOLDERS.append(item)
                scan(item)
            continue
...
def main(folder: Path):
    ...
    for file in APLICATIONS:
        handle_media(file, folder / 'aplications' / 'EXE')        
